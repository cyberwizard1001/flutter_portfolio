import 'package:flutter/material.dart';

class SiteColors{
  static const Color primaryDark = Color(0xffffffff);
  static const Color primaryInverseDark = Color(0xFF000000);
  static const Color secondaryDark = Color(0xFFC4C4C4);
  static const Color backgroundDark = Color(0xFF000000);
  static const Color secondaryBackgroundDark = Color(0xFF141414);
  static const Color focusBackgroundDark= Color(0xff797979);
  static const Color focusDark = Color(0xffB9E320);
  static const Color terminalBackgroundDark = Color(0xFF1C1F28);
  static const Color terminalPillBackgroundDark = Color(0xFF687363);
  static const Color cardBackgroundColorDark = Color(0xff1C1C22);
}